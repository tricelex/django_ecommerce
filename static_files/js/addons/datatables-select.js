  ...
  * cells: a `_selected_cells` property which contains an array of boolean values
  ...
  function enableMouseSelection(dt) {
    ...
    container.on('mousedown.dtSelect', selector, function(e) {
      ...
      if ($(e.target).closest('div.' + wrapperClass)[0] !=
          dt.table().container()) {
        return;
      }
  ...
  function init(ctx) {
    ...
    api.on('draw.dtSelect.dt select.dtSelect.dt deselect.dtSelect.dt info.dt',
        function() {
          info(api);
        });
...
// reduce the code size
  ...
  DataTable.ext.selector.cell.push(function(settings, opts, cells) {
    ...
    for (var i = 0, ien = cells.length; i < ien; i++) {
      ...
      if ((selected === true && rowData._selected_cells &&
          rowData._selected_cells[cells[i].column] === true) ||
          (selected === false && (!rowData._selected_cells ||
...
// Common events with suitable namespaces
  ...
  function enabled(dt, config) {
    if ($.inArray('rows', config.limitTo) !== -1 &&
        dt.rows({selected: true}).any()) {
      return true;
    }

    if ($.inArray('columns', config.limitTo) !== -1 &&
        dt.columns({selected: true}).any()) {
      return true;
    }

    if ($.inArray('cells', config.limitTo) !== -1 &&
