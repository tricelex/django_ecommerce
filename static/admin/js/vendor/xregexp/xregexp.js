 ...
 */
        ...
        module.exports = function(XRegExp) {
          ...
          var parts = XRegExp.union([/\({{([\w$]+)}}\)|{{([\w$]+)}}/, subParts],
              'g', {
                conjunction: 'or',
              });
          ...
          function deanchor(pattern) {
            ...
            if (
              ...
              return pattern.replace(leadingAnchor, '').
          ...
          XRegExp.build = function(pattern, subs, flags) {
            ...
            var output = patternAsRegex.source.replace(parts,
                function($0, $1, $2, $3, $4) {
                  var subName = $1 || $2;
                  var capName;
                  var intro;
                  ...
                  if (subName) {
                    ...
                    return intro + data[subName].pattern.replace(subParts,
                        function(match, paren, backref) {
                          // Capturing group
                          if (paren) {
                            capName = data[subName].names[numCaps -
                            numPriorCaps];
                            ++numCaps;
        ...
        module.exports = function(XRegExp) {
          ...
          XRegExp.matchRecursive = function(str, left, right, flags, options) {
                ...
                if (!--openTokens) {
                  if (vN) {
                    if (vN[0] && outerStart > lastOuterEnd) {
                      output.push(
                          row(vN[0], str.slice(lastOuterEnd, outerStart),
                    ...
                    if (vN[3]) {
                      output.push(row(vN[3], str.slice(delimStart, delimEnd),
                          delimStart, delimEnd));
                    }
                  } else {
            ...
            if (global && !sticky && vN && vN[0] && str.length > lastOuterEnd) {
              output.push(row(vN[0], str.slice(lastOuterEnd), lastOuterEnd,
                  str.length));
            }
...
// Tracker for known flags, including addon flags
        ...
        function copyRegex(regex, options) {
          ...
          if (flagsToRemove) {
            flags = nativ.replace.call(flags,
        ...
        function getNativeFlags(regex) {
          return hasFlagsProp ?
              ...
              nativ.exec.call(/\/([a-z]*)$/i,
                  RegExp.prototype.toString.call(regex))[1];
        }
        ...
        function isPatternNext(pattern, pos, flags, needlePattern) {
          ...
          return nativ.test.call(
              new RegExp('^(?:' + patternsToIgnore.join('|') + ')*(?:' +
                  needlePattern + ')'),
              pattern.slice(pos),
          );
        }
        ...
        function prepareFlags(pattern, flags) {
          ...
          pattern = nativ.replace.call(pattern, /^\(\?([\w$]+)\)/,
              function($0, $1) {
          ...
          if (!patternCache[pattern][flags]) {
            ...
            while (pos < appliedPattern.length) {
              do {
                ...
                result = runTokens(appliedPattern, appliedFlags, pos, scope,
                    context);
              ...
              if (result) {
                ...
                var token = XRegExp.exec(appliedPattern, nativeTokens[scope],
                    pos, 'sticky')[0];
                output += token;
                pos += token.length;
...
// Intentionally undocumented; used in tests
        ...
        XRegExp.match = function(str, regex, scope) {
          ...
          var cacheKey = ((global ? 'g' : '') + (regex.sticky ? 'y' : '')) ||
              'noGY';
          var result;
          var r2;
          ...
          return (function recurseChain(values, level) {
            ...
            function addMatch(match) {
              if (item.backref) {
                ...
                // bounds of the array.
                if (!(match.hasOwnProperty(item.backref) || +item.backref <
                    match.length)) {
                  throw new ReferenceError(
        ...
        XRegExp.replace = function(str, search, replacement, scope) {
          ...
          var cacheKey = ((global ? 'g' : '') + (search.sticky ? 'y' : '')) ||
              'noGY';
          var s2 = search;
          var result;
        ...
        XRegExp.union = function(patterns, flags, options) {
          ...
          for (var i = 0; i < patterns.length; ++i) {
            ...
            if (XRegExp.isRegExp(pattern)) {
              ...
              captureNames = (pattern[REGEX_DATA] &&
                  pattern[REGEX_DATA].captureNames) || [];

              // Rewrite backreferences. Passing to XRegExp dies on octals and ensures patterns are
              // independently valid; helps keep this simple. Named captures are put back
              output.push(
                  nativ.replace.call(XRegExp(pattern.source).source, parts,
                      rewrite));
            } else {
...
// ==--------------------------==
        ...
        fixed.exec = function(str) {
          ...
          if (match) {
            ...
            if (!correctExecNpcg && match.length > 1 && indexOf(match, '') >
                -1) {
              r2 = copyRegex(this, {
                removeG: true,
                isInternalOnly: true,
              ...
              nativ.replace.call(String(str).slice(match.index), r2,
                  function() {
        ...
        fixed.replace = function(search, replacement) {
          ...
          if (isType(replacement, 'Function')) {
            ...
            result = nativ.replace.call(this == null ? this : String(this),
                search, function() {
                  // Keep this function's `arguments` available through closure
                  var args = arguments;
                  return nativ.replace.call(String(replacement),
                      replacementToken, function($0, $1, $2) {
                        var n;
